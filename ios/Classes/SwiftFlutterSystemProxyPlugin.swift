...
public class SwiftFlutterSystemProxyPlugin: NSObject, FlutterPlugin {
    ...
    static func resolve(url: String) -> [String: Any]? {
        ...
        let proxConfigDict =
            CFNetworkCopySystemProxySettings()?.takeUnretainedValue() as NSDictionary?
    ...
    static func handlePacContent(pacContent: String, url: String) {
        let proxies =
            CFNetworkCopyProxiesForAutoConfigurationScript(
                pacContent as CFString,
                CFURLCreateWithString(kCFAllocatorDefault, url as CFString, nil), nil)!
            .takeUnretainedValue() as? [[CFString: Any]] ?? []
        if proxies.count > 0 {
            let proxy = proxies.first {
                $0[kCFProxyTypeKey] as! CFString == kCFProxyTypeHTTP
                    || $0[kCFProxyTypeKey] as! CFString == kCFProxyTypeHTTPS
            }
            if proxy != nil {
    ...
    static func handlePacUrl(pacUrl: String, url: String) {
        ...
        if pacUrl != nil && targetUrl != nil {
            var context: CFStreamClientContext = CFStreamClientContext.init(
            ...
            let runLoopSource = CFNetworkExecuteProxyAutoConfigurationURL(
                _pacUrl!, targetUrl!,
                { client, proxies, error in
                    let _proxies = proxies as? [[CFString: Any]] ?? []
                    if _proxies != nil {
                        if _proxies.count > 0 {
                            let proxy = _proxies.first {
                                $0[kCFProxyTypeKey] as! CFString == kCFProxyTypeHTTP
                                    || $0[kCFProxyTypeKey] as! CFString == kCFProxyTypeHTTPS
                            }
                            if proxy != nil {
